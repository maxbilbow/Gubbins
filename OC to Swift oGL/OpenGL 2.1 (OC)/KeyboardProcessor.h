...
void repeatedKeys(){
    ...
    if (keys.keySpecialStates[0/*GLUT_KEY_UP*/])
        if (keys.keyStates[9])
            [sun lightUp:1];
        else
            [observer extendArmLength:1];

    else if(keys.keySpecialStates[0/*GLUT_KEY_DOWN*/])
        if (keys.keyStates[9])
            [sun lightUp:-1];
        else
            [observer extendArmLength:-1];
    ...
    switch (key)
    {
        ...
        case 'm':
            ...
            if ( [observer hasFocus]){
                //center();
                NSLog(@"glutSetCursor(GLUT_CURSOR_NONE);");
                ...
                NSLog(@"glutSetCursor(GLUT_CURSOR_INHERIT);");
            }
            break;
        case 32:
    // [observer stop();
